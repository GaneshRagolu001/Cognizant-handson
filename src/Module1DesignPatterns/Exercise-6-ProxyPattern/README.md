...
### Why is the Proxy Pattern needed for video loading?

Loading videos from a remote server is an **expensive operation**:

| Operation | Cost |
|-----------|------|
| Network connection | High (latency, bandwidth) |
| Download video data | High (depends on file size) |
| Store in memory | Medium (RAM consumption) |
| Display on screen | Low (fast operation) |

**Without Proxy:**
```java
// Video loads IMMEDIATELY from the server -- even if we
// never actually display it. Resources wasted!
Video img = new RealVideo("tutorial.mp4");
...
Video img = new ProxyVideo("tutorial.mp4");
...
**Benefits:**
- Loading is **deferred** until the video is actually needed
- Once loaded, the video is **cached** and never loaded again
- If `display()` is never called, the video is **never loaded** (zero cost)
...
|-- RealVideo.java          <-- Real Subject (expensive)
|-- ProxyVideo.java         <-- Proxy (lazy init + cache)
...
```
Client --> ProxyVideo.display()
               |
               +-- Is realVideo null?
               |
          YES  |              NO
               v               v
     Create RealVideo     Skip creation
...
### Real Subject -- `RealVideo`

The **Real Subject** is the actual object that performs the core work. Creating a RealVideo triggers an expensive loading operation (simulated by a print statement).

| Role | Details |
|------|---------|
| Implements | `Video` |
| Constructor | Accepts filename, loads from remote server |
| display() | Prints "Displaying video: \<filename\>" |
| Cost | Expensive to create (remote loading) |

### Proxy -- `ProxyVideo`

The **Proxy** stands in for the RealVideo and controls access to it. It provides lazy initialization (defer creation) and caching (create only once).

| Role | Details |
|------|---------|
| Implements | `Video` |
| Contains | `private RealVideo realVideo` (initially null) |
| Constructor | Stores filename only (cheap -- no loading) |
| display() | Creates RealVideo on first call, caches for reuse |

### Client -- `ProxyPatternTest`

The **Client** works with the `Video` interface. It creates a `ProxyVideo` and calls `display()` multiple times to demonstrate lazy initialization and caching.
...
```
=============================================
      Proxy Pattern - Video Viewer            
=============================================

--- Creating ProxyVideo (no loading yet) ---
ProxyVideo created. Video NOT loaded yet.

--- First call to display() ---
Loading video from remote server: tutorial.mp4
Displaying video: tutorial.mp4

--- Second call to display() (cached) ---
Displaying video: tutorial.mp4

--- Another ProxyVideo ---
ProxyVideo created for movie.mkv.

--- First call to display() ---
Loading video from remote server: movie.mkv
Displaying video: movie.mkv
...
**Key observation:** "Loading video from remote server" appears only **once per video**, proving that the proxy caches the RealVideo after the first load.
...
| **Transparency** | The client works with the `Video` interface and has no idea it's using a proxy. The proxy is a drop-in replacement. |
| **Separation of Concerns** | Resource management logic (lazy loading, caching) is in the proxy, keeping the RealVideo class clean and focused. |
| **Open/Closed Principle** | New proxies (e.g., logging, security) can be added without modifying RealVideo or client code. |
...
|------|---------|---------|
| **Virtual Proxy** | Delays creation of expensive objects until needed | Our `ProxyVideo` (lazy loading) |
...
| **CDN (Content Delivery Network)** | A CDN acts as a caching proxy for web content. It serves cached copies of videos/pages instead of hitting the origin server every time. |
| **Browser Video Loading** | Browsers use placeholder proxies for videos. They show a loading spinner (proxy) until the actual video data is downloaded (real object). |
...
| **Example** | ProxyVideo delays loading | PayPalAdapter converts API | SMSDecorator adds SMS |
...
**A:** Lazy initialization means delaying the creation of an object until it is actually needed. In our example, the `RealVideo` is not created when `ProxyVideo` is constructed -- it is created only when `display()` is called for the first time. This saves resources if the object is never used.

---

### Q3: How does caching work in the Proxy Pattern?
**A:** After the real object is created for the first time, the proxy stores (caches) a reference to it. On subsequent calls, the proxy reuses the cached object instead of creating a new one. In our example, `realVideo` is set to `null` initially, created on the first `display()` call, and reused on all subsequent calls.
...
### Q6: Why is the RealVideo constructor expensive?
**A:** In our simulation, the constructor calls `loadFromRemoteServer()` which represents a costly operation like downloading video data over the network. In production, this could involve opening HTTP connections, transferring megabytes of data, and storing them in memory.

---

### Q7: What happens if display() is never called on ProxyVideo?
**A:** The RealVideo is **never created**. The proxy simply holds the filename string and nothing else. This is the key benefit of lazy initialization -- you pay zero cost for objects you never use.
