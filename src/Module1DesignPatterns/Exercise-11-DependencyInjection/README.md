...
This exercise demonstrates the use of **Constructor Injection** to decouple a service class (`EmployeeService`) from its data access layer (`EmployeeRepository`).
...
1. **Difficulty in Testing:** You cannot easily test `EmployeeService` in isolation because it is hardwired to `EmployeeRepositoryImpl` (which might connect to a real database). Mocking the repository is extremely difficult.
2. **Lack of Flexibility:** If you want to switch from `EmployeeRepositoryImpl` (e.g., SQL database) to `NoSqlEmployeeRepositoryImpl`, you must modify the source code of `EmployeeService`.
3. **Violates Dependency Inversion Principle (DIP):** High-level modules (`EmployeeService`) depend directly on low-level modules (`EmployeeRepositoryImpl`) instead of abstractions.

**With Dependency Injection (Loose Coupling):**
- The service depends on an interface (`EmployeeRepository`).
...
| **Dependency Type** | Depends on concrete classes (`EmployeeRepositoryImpl`). | Depends on interfaces (`EmployeeRepository`). |
...
|
|-- EmployeeRepository.java       <-- Dependency Interface (Abstraction)
|-- EmployeeRepositoryImpl.java   <-- Concrete Dependency (Implementation)
|-- EmployeeService.java          <-- Service Class (Client receiving Injection)
...
|-----------------------------------|            | + findEmployeeById(id: int):String|
| + EmployeeService(repository:     |            +-----------------^-----------------+
|     EmployeeRepository)           |                              |
| + findEmployeeById(id: int):String|                              | implements
+-----------------------------------+                              |
                                                 +-----------------+-----------------+
...
+------------------------+      +---------------------+             +-----------------+
| EmployeeRepositoryImpl |      |   EmployeeService   |             | Client (Test)   |
...
| :--- | :--- | :--- | :--- | :--- |
| **Constructor Injection** | Dependencies are passed through the class constructor. | **Mandatory** & Immutable dependencies. | `public EmployeeService(EmployeeRepository repo)` | **Pros:** Immutable fields (can use `final`), guarantees all dependencies are present at construction, easier to test. |
| **Setter Injection** | Dependencies are passed through setter methods. | **Optional** or changeable dependencies. | `public void setRepository(EmployeeRepository repo)` | **Pros:** Can change dependency later.<br>**Cons:** Object can exist in a half-configured state; fields cannot be `final`. |
| **Field Injection** | Dependencies are injected directly into fields via reflection. | Framework-driven boilerplate reduction (e.g., Spring). | `@Autowired private EmployeeRepository repo;` | **Pros:** Clean code with no boilerplate.<br>**Cons:** Tight coupling with DI framework, hard to test without framework, fields cannot be `final`. |
...
```
=============================================
   Dependency Injection Pattern - Employee App
=============================================
...
--- Testing with a non-existent employee ID ---
Employee with ID 999 not found in the repository.
...
In this exercise, `EmployeeService` (high-level) depends on the interface `EmployeeRepository` (abstraction), not `EmployeeRepositoryImpl` (detail).
...
1. Instantiating the dependency (`EmployeeRepositoryImpl`).
2. Instantiating the client (`EmployeeService`).
...
**A:** A Mock object is a dummy implementation of an interface that simulates real behavior (e.g., returning predefined data without querying a database) for testing purposes. Because `EmployeeService` accepts any `EmployeeRepository` interface implementation in its constructor, we can write a test that passes a Mock repository directly:
```java
// Testing EmployeeService using a Mock repository
EmployeeRepository mockRepo = Mockito.mock(EmployeeRepository.class);
Mockito.when(mockRepo.findEmployeeById(101)).thenReturn("Mocked User");

EmployeeService service = new EmployeeService(mockRepo); // Inject Mock!
assertEquals("Mocked User", service.findEmployeeById(101));
```
