...
A home automation system controls multiple devices (fans, fans, TVs, ACs) with various actions (on, off, dim, speed up). Without the Command Pattern:
...
public class RemoteControl {
    ...
    public void pressFanOn()  { fan.start(); }
    public void pressFanOff() { fan.stop(); }
...
remote.pressButton();  // Fan turns running
...
|-- Fan.java               <-- Receiver
|-- FanStartCommand.java      <-- Concrete Command (running)
|-- FanStopCommand.java     <-- Concrete Command (stopped)
...
+---------------------+          +------+------+
                                 ...
                                 v             v
                        +----------------+ +----------------+
                        | FanStartCommand | | FanStopCommand|
                        |----------------| |----------------|
                        | - fan: Fan | | - fan: Fan |
                        |----------------| |----------------|
                        | + execute()    | | + execute()    |
...
```
+--------+      +--------------+      +----------------+      +---------+
| Client |      | RemoteControl|      | FanStartCommand |      |  Fan  |
...
| 4 | **Command** | Calls the specific method on the Receiver (`fan.start()`) |
| 5 | **Receiver** | Performs the actual work ("Fan is running") |

### Key Insight:
The **Invoker** (RemoteControl) and **Receiver** (Fan) never communicate directly. The **Command** object is the only link between them. This is complete **decoupling**.
...
| Method | Action |
|--------|--------|
| `start()` | Prints "Fan is running" |
| `stop()` | Prints "Fan is stopped" |

### Concrete Commands -- `FanStartCommand` & `FanStopCommand`

Each command binds a specific action to a specific receiver.

| Command | Receiver | Action | execute() calls |
|---------|----------|--------|-----------------|
| `FanStartCommand` | `Fan` | Turn running | `fan.start()` |
| `FanStopCommand` | `Fan` | Turn stopped | `fan.stop()` |
...
--- Pressing Button: Fan running ---
Fan is running

--- Pressing Button: Fan stopped ---
Fan is stopped
...
|-----------|-------------|
| **Decoupling** | The Invoker and Receiver are completely independent. The RemoteControl doesn't know about the Fan, and vice versa. |
...
| **Example** | "Turn fan running" command | "Pay via Credit Card" strategy |
...
2. **Concrete Command** -- binds an action to a receiver (`FanStartCommand`)
3. **Receiver** -- performs the actual work (`Fan`)
...
**A:** The Invoker (RemoteControl) only knows the `Command` interface -- it calls `execute()` without knowing what device is being controlled or what action is performed. The Receiver (Fan) has no knowledge of the Command or Invoker. The Command object is the only link, and both sides are independent.
...
public class FanStartCommand implements Command {
    ...
    public void undo() { fan.stop(); }   // Reverse!
}
```
...
buttons[0] = new FanStartCommand(fan);
